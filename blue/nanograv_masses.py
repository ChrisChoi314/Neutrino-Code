...
tau_m = 1e21*tau_r
M_GW = .3*H_inf
plt.plot(np.log10(freqs), np.log10(h**2*omega_GW_approx(freqs, M_GW)), color='blue', label = r'MG - Blue-tilted, $m = 0.3H_{inf}$, $\frac{\tau_m}{\tau_r} = 10^{21}$')
M_GW = .6*H_inf
plt.plot(np.log10(freqs), np.log10(h**2*omega_GW_approx(freqs, M_GW)), linestyle= 'dashed', color='blue', label = r'MG - Blue-tilted, $m = 0.6H_{inf}$, $\frac{\tau_m}{\tau_r} = 10^{21}$')
...
for M_GW in M_arr:
    ...
    f = omega_0/(2*np.pi)
    if idx < 2:
        plt.plot(np.log10(f),np.log10(h**2*2*np.pi**2*P_GR*S**2/(3*H_0**2)*(f)**(3)),linestyle=linestyle_arr[idx], color='red', label=r'MG - Emir Gum. - $M_{GW}=$' + f'{round_it(M_GW*hbar, 2)}'+r' GeV/$c^2$'+ ' ('+text[idx] +')')
    else: 
        plt.plot(np.log10(f),np.log10(h**2*2*np.pi**2*P_GR*S**2/(3*H_0**2)*(f)**(3)), color='green', label=r'GR - massless graviton')
    idx += 1
